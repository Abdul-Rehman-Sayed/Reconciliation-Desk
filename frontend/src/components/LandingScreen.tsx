    ...
    <header className="border-b border-rule bg-sheet">
      <div className="mx-auto max-w-[1080px] px-4 py-14 sm:px-6 sm:py-20">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          >
            ...
            <p className="mt-5 max-w-[64ch] text-[15px] leading-relaxed text-slate sm:text-[16px]">
              Settlements land three days late. The gateway takes its fee before
              the money arrives. A webhook fires twice. Four payments settle as
              one credit that matches none of them. Today a person works through
              all of it by hand, every month.
            </p>
            <p className="mt-3 max-w-[64ch] text-[15px] leading-relaxed sm:text-[16px]">
              This automates the part that is mechanical, explains the part that
              is not, and{" "}
              <span className="font-medium text-ink">
                refuses to touch the money either way.
              </span>
            </p>
        ...
        >
          <HeroFigure
            value="90.19%"
            label="resolved by rule"
            note="no human, no model"
          />
          <HeroFigure
            value="< 10%"
            label="reaches the model"
            note="84 of 856 records"
          />
          <HeroFigure
            value="100%"
            label="auto-resolve precision"
            note="0 wrong commits"
          />
          <HeroFigure
            value="<0.1 s"
        ...
        <p className="num mt-4 text-[10px] leading-relaxed text-mute">
          Measured on the bundled standard set (856 records)
        </p>
      </div>
    </header>
  );
  ...
  return (
    <section className="mx-auto max-w-[1080px] px-4 py-14 sm:px-6 sm:py-16">
      ...
      <p className="mt-2 max-w-[68ch] text-[14px] leading-relaxed text-slate">
        The split is visible in the API on purpose. Reconciling returns in about
        100 ms without touching a model; explaining is a separate call you have
        to make. The interface runs them in that order, so what you watch on
        screen is the order the work actually happened in.
      </p>
  ...
  return (
    <section className="border-y border-rule bg-sheet">
      <div className="mx-auto max-w-[1080px] px-4 py-14 sm:px-6 sm:py-16">
        ...
        <p className="mt-2 max-w-[68ch] text-[14px] leading-relaxed text-slate">
          The adversarial set keeps every category the brief specifies and adds
          four the thresholds were never designed around. The gap between 83%
          and 99% there is 141 records the engine found a candidate for and
          deliberately refused to commit — which is the behaviour you want from
        ...
        <p className="num mt-3 text-[10px] leading-relaxed text-mute">
          A naive reference-and-amount join scores 76.64% recall on the same
          data
        </p>
      </div>
    </section>
...
const LIMITS = [
  ...
  "Ground truth exists only for the two bundled datasets. The tolerances are validated against unseen data rather than fitted to a single batch.",
    ...
    <section className="mx-auto max-w-[1080px] px-4 py-14 sm:px-6 sm:py-16">
      <div className="flex items-start gap-2.5">
        ...
        <div>
          ...
          <p className="mt-2 max-w-[68ch] text-[14px] leading-relaxed text-slate">
            Real bank exports are messier than anything we thought to generate,
            and the failure modes we did not imagine are exactly the ones not in
            here.
          </p>
        </div>
      </div>
